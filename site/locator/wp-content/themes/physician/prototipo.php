  ...
  <div id="column-content">
    <div class="post" id="post-<?php the_ID(); ?>">
      ...
      if( isset($_GET['unsuscribe']) ) :
        ...
        <div class="<?php post_class(); ?>" id="post-<?php the_ID(); ?>">
          <div class="ctotal">
            <p class="hay">
              <?php if($yaenvio) 
                echo "You have been unsubscribed from our physician notification list."; 
              else 
                echo "Your mail was not in our physician notification list.";
              ?>
            </p>
          </div>  
        </div>
      ...
      if ( !empty($_POST['submitnf'])) : ?>
        ...
        if(!$update) {
          $user_count = $wpdb->get_var("SELECT COUNT(*) FROM datauserword 
                WHERE Email ='".$_POST['emailnf']."' AND Zip = ".$_POST['zipnf']);
          if(!$user_count) $ban= $wpdb->insert('datauserword', 
                array('Zip' => $_POST['zipnf'], 
                      'Distance' => $_POST['distancenf'], 
                      'Email' => $_POST['emailnf'], 
                      'Name' => $_POST['namenf']), 
                array('%s','%d', '%s','%s'));
        ...
        if (count($coordes)) { 
          ...
          foreach ($coordes as $coords) 
            $bans = $wpdb->insert( 'searching', 
                    ...
                    array('%s','%s', '%d', '%s', '%s'));
          //CONTINUA BUSQUEDA
          $founds_zips = $wpdb->get_results("SELECT Zipcode, 
             MIN( 3959 * acos( cos( radians( ".$coords->Lat." ) ) * 
              cos( radians( Lat ) ) * cos( radians( Lon ) - 
              radians( ".$coords->Lon." ) ) + sin( radians( ".$coords->Lat." ) ) * 
              sin( radians( Lat ) ) ) ) AS distance 
             FROM zipcodes GROUP BY Zipcode 
              ...
              while ($query->have_posts()) : $query->the_post(); 
                if (!in_array(get_the_ID(), $encontrados)) {
                  ...
                  if (strlen($location['address'])>1)
                    ...
                    $direc = $location;
                  $found_d= $wpdb->get_var("SELECT ( 3959 * 
                     acos( cos( radians( ".$coords->Lat." ) ) * 
                     cos( radians( Lat ) ) * cos( radians( Lon ) 
                     - radians( ".$coords->Lon." ) ) 
                     + sin( radians( ".$coords->Lat." ) ) 
                     * sin( radians( Lat ) ) ) ) AS distance 
                     FROM zipcodes 
                     WHERE zipcode=".get_field('zip'));
                  $arr_post = array();
                  $arr_post['class'] = get_post_class();
                  $arr_post['ID'] = get_the_ID();
                  $arr_post['distance'] = $found_d;
                  $arr_post['directions'] = $direc;
                  for ($i=1;$i<6;$i++) { 
                    if (get_field('first_name_'.$i) <> '') { 
                      $arr_post['others'][$i]['first_name'] = get_field('first_name_'.$i);
                      $arr_post['others'][$i]['last_name'] = get_field('last_name_'.$i); 
                      $arr_post['others'][$i]['designation'] = get_field('designation_'.$i);
                    }
                  }
                  $arr_post['practice_name'] = get_field('practice_name'); 
                  $arr_post['address'] = get_field('address'); 
                  $arr_post['suite'] = get_field('suite'); 
                  $arr_post['city'] = get_field('city'); 
                  $arr_post['zip'] = get_field('zip'); 
                  $arr_post['state'] = get_field('state'); 
                  $arr_post['phone_number'] = get_field('phone_number'); 
                  $arr_post['email_address'] = get_field('email_address'); 
                  $arr_post['website'] = get_field('website'); 
                  $arr_post['address_line'] = get_field('address_line'); 
                  $arr_post['coordinates'] = $location['coordinates']; 
                  $arr_post['lat'] = $lat; 
                  $arr_post['lng'] = $lng; 
                  array_push($arr_posts, $arr_post);
                }
              endwhile; 
              array_sort_bycolumn($arr_posts,'distance'); 
                  /*echo '<pre>';
                    print_r($arr_posts);
                  echo '</pre>';*/
              foreach($arr_posts as $p): ?>
                <div <?=$p['class'] ?> id="post-<?=$p['ID'] ?>">
                  <div class="entry">
                  <!--fichas de los medicos-->
                    <div class="theoffice">
                      <div class="mapgoogle">
                        <div id="map_<?=$p['ID'] ?>" class="mapagoogle"></div><!--$fzip->distance-->
                        <p id="pdistance_<?=$p['ID'] ?>">
                          <strong>Distance: <?php echo round( $p['distance'] * 100) / 100; ?> miles</strong>
                          <a href="https://maps.google.com/maps?q=<?=$p['directions'] ?>&hl=en&t=h&mra=ls&z=16&layer=t" 
                            target="_blank" style="float:right;">Directions</a>
                        </p>
                      </div>
                      <div class="names">
                        <?php for ($i=1;$i<6;$i++) { 
                      ...
                      <?php if (!empty($p['practice_name'])): ?>
                        <span><!--Practice Name:--> <?=$p['practice_name'] ?></span>
                      <?php endif; ?>
                      ...
                      <div style="margin-top:20px; margin-bottom:18px;">
                        <span>
                          <?php if (!empty($p['phone_number'])) { ?>
                            Tel: <?=$p['phone_number'] ?><br />
                          <?php } ?>
                          <?php if (!empty($p['email_address'])) { ?>
                            Email: <span style="display:inline; color:#fcac00"><a href="mailto:<?=$p['email_address'] ?>"><?=$p['email_address'] ?></a></span><br /><? } ?>
        ...
        <?php if ( (count($founds_zips)>0 && $bant == 0) ||  count($coordes)== 0) :?>
          <div style="width:770px; margin-right:auto; margin-left:auto;">
            ...
            <p style="display:block; margin-top:10px; margin-bottom:40px; font-size:16px; line-height:24px;">
              ...
              up for an email notification once there is a PROPEL physician in your area.
            </p>
