...
<?php get_header(); ?>
        ...
        <p>FIND A PROPEL PHYSICIAN</p>
               ...
               <!-- <div id="header-strip">
					<img src="_img/_content/header-strip.jpg" />
		...
		<div class="post" id="post-<?php the_ID(); ?>"><a name="search" id="search"></a>
        <div style=" width:95%; background-color:#fcac00; padding-left:50px; padding-top:16px; padding-bottom:8px; font-family: Helvetica, Arial;">
         ...
         <div style="float:left;  width:191px;">
          <span class="moverlabeldos"><input type="text" name="zipcode" id="zipcode" class="campouno" placeholder="Zip code*" value="<?=$_GET['zipcode']?>"></span>
    	</div>	  
		...
		$encontrados=array();				
			...
			$query = new WP_Query($args);
				...
				$bant++;
                    ...
                    <p id="pdistance_<?php the_ID(); ?>"><strong>Distance: <?php echo round( $fd->distance * 100) / 100; ?> miles</strong><a href="https://maps.google.com/maps?q=<?php echo $direc;?>&hl=en&t=h&mra=ls&z=16&layer=t" target="_blank" style="float:right;">Directions</a></p>
                    </div>
<div class="names">
						<?php 
						for ($i=1;$i<6;$i++) { 
					...
					<span><!--Address:--><?php the_field('address'); ?><? if(!empty($suite)) { echo '<br>' . $suite; } ?><br><? the_field('city') ?>, <? the_field('state')?> <? the_field('zip')?></span>
			...
			<?php 
				...
				<div style="width:770px; margin-right:auto; margin-left:auto;">
								...
								<p style="display:block; margin-top:10px; margin-bottom:40px; font-size:16px; line-height:24px;">Please widen your search distance, or fill out the form below to sign up for an email notification once there is a PROPEL physician in your area.</p>
							...
							<div class="left-content-header">
								...
								<p style="margin-bottom:10px; margin-left:2px;">Fill out this form to receive an email notification once a PROPEL physician is in your area.</p>
					<div id="contentmail">
	...
	<?if ( !empty($_POST['submitnf']) ) {  ?>
		<span><h1 style="font-size:18px;  margin-left:auto; margin-right:auto; line-height:24px">Thank you for signing up. We will send you an email notification when a physician is added in your area.</h1></span>
	<? } ?>
